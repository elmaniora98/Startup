...
class ApproveAuctionTest(TestCase):
    ...
    def test_creates_audit_log(self):
        self.client.login(username='admin', password='admin123')
        now = timezone.now()
        self.client.post(reverse('auctions:approve_auction', kwargs={'pk': self.pending.id}), {'start_at': (now+timedelta(days=2)).isoformat(), 'end_at': (now+timedelta(days=3)).isoformat()})
