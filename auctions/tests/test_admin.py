...
class ValidationQueueTest(TestCase):
    ...
    def test_only_pending(self):
        self.client.login(username='admin', password='admin123')
        response = self.client.get(reverse('validation_queue'))
...
class ApproveAuctionTest(TestCase):
    ...
    def test_valid_approval(self):
        self.client.login(username='admin', password='admin123')
        now = timezone.now()
        response = self.client.post(reverse('approve_auction', kwargs={'pk': self.pending.id}), {'start_at': (now+timedelta(days=2)).isoformat(), 'end_at': (now+timedelta(days=3)).isoformat()}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.SCHEDULED)
    def test_creates_audit_log(self):
        self.client.login(username='admin', password='admin123')
        now = timezone.now()
        self.client.post(reverse('approve_auction', kwargs={'pk': self.pending.id}), {'start_at': (now+timedelta(days=2)).isoformat(), 'end_at': (now+timedelta(days=3)).isoformat()})
    ...
    def test_end_before_start_error(self):
        self.client.login(username='admin', password='admin123')
        now = timezone.now()
        response = self.client.post(reverse('approve_auction', kwargs={'pk': self.pending.id}), {'start_at': (now+timedelta(days=3)).isoformat(), 'end_at': (now+timedelta(days=2)).isoformat()}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.PENDING)
    def test_start_in_past_error(self):
        self.client.login(username='admin', password='admin123')
        now = timezone.now()
        response = self.client.post(reverse('approve_auction', kwargs={'pk': self.pending.id}), {'start_at': (now-timedelta(days=1)).isoformat(), 'end_at': (now+timedelta(days=1)).isoformat()}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.PENDING)
...
class RejectAuctionTest(TestCase):
    ...
    def test_valid_rejection(self):
        self.client.login(username='admin', password='admin123')
        response = self.client.post(reverse('reject_auction', kwargs={'pk': self.pending.id}), {'rejection_reason': 'Cette enchere ne respecte pas les criteres de qualite.'}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.REJECTED)
    def test_creates_audit_log(self):
        self.client.login(username='admin', password='admin123')
        self.client.post(reverse('reject_auction', kwargs={'pk': self.pending.id}), {'rejection_reason': 'Cette enchere ne respecte pas les criteres de qualite.'})
    ...
    def test_creates_notification(self):
        ...
        self.client.post(reverse('reject_auction', kwargs={'pk': self.pending.id}), {'rejection_reason': reason})
    ...
    def test_empty_reason_error(self):
        self.client.login(username='admin', password='admin123')
        response = self.client.post(reverse('reject_auction', kwargs={'pk': self.pending.id}), {'rejection_reason': ''}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.PENDING)
    def test_short_reason_error(self):
        self.client.login(username='admin', password='admin123')
        response = self.client.post(reverse('reject_auction', kwargs={'pk': self.pending.id}), {'rejection_reason': 'Court'}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.PENDING)
    def test_cannot_reject_non_pending(self):
        ...
        self.pending.save()
        self.client.login(username='admin', password='admin123')
        response = self.client.post(reverse('reject_auction', kwargs={'pk': self.pending.id}), {'rejection_reason': 'Motif'}, follow=True)
        self.pending.refresh_from_db()
        self.assertEqual(self.pending.status, Auction.Status.LIVE)
